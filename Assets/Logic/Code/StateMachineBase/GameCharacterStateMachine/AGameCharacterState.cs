...
public abstract class AGameCharacterState : IState<EGameCharacterState>
{
	...
	protected void CombatMovement(float deltaTime, float initYVelocity, float initXVelocity, ref float lerpTimeY, ref float lerpTimeX, ref float currentYPosAnimCurve, bool isGrounded)
	{
		...
		float xMotion = isValidHit ? 0 : Mathf.Clamp(GameCharacter.MovementComponent.RootmotionVector.x + Mathf.Lerp(initXVelocity, 0, lerpTimeX), int.MinValue /*-GameCharacter.GameCharacterData.MaxMovementSpeed*/, int.MaxValue /*GameCharacter.GameCharacterData.MaxMovementSpeed*/);
