...
public class BlitMaterialFeature : ScriptableRendererFeature
{
	class RenderPass : ScriptableRenderPass
	{
		...
		public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
		{
			...
			//int myLayerIndexVariable = 2; // ...or LayerMask.NameToLayer(myLayerName)
			int myLayerIndexVariable = LayerMask.NameToLayer("Character");
			int myLayerMaskVariable = 1 << myLayerIndexVariable;
			RenderQueueRange myRenderQueueRange = RenderQueueRange.opaque;
			FilteringSettings filters = new FilteringSettings(myRenderQueueRange, myLayerMaskVariable);

			//var drawSettings = CreateDrawingSettings(m_shaderTag, ref renderingData, SortingCriteria.None);
			var ds = new DrawingSettings();
			context.DrawRenderers(renderingData.cullResults, ref ds, ref filters);
