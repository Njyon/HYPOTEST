...
Shader "Hidden/AkiDevCat/AVL/RenderingPass"
{
    ...
    float4 frag_render(v2f input) : SV_Target
    {
        ...
        float3 viewPlane = (input.ray.xyz - _WorldSpaceCameraPos) / dot(input.ray.xyz - _WorldSpaceCameraPos, unity_WorldToCamera._m20_m21_m22);

        depth *= length(viewPlane);
        
        float3 rayOrigin = input.ray.xyz;
        float3 rayDir = normalize(input.ray.xyz - _WorldSpaceCameraPos);
